...
implements IFloodlightModule, ILearningSwitchService, IOFMessageListener, IControllerCompletionListener {
	...
	private Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		if (outPort == null) {
			...
			//this.doflood_me(sw,pi);
			OFPort inPort2 = OFMessageUtils.getInPort(pi);
			OFPacketOut.Builder pob = sw.getOFFactory().buildPacketOut();
			List<OFAction> actions = new ArrayList<OFAction>();

			ArrayList<OFPort> arrBroadcastPorts = new ArrayList<OFPort>();
			if(sw.getId().getLong() == 1){
				arrBroadcastPorts.add(OFPort.ofInt(1));
				arrBroadcastPorts.add(OFPort.ofInt(2));
				arrBroadcastPorts.add(OFPort.ofInt(3));
			}
			else if(sw.getId().getLong() == 2) {
				arrBroadcastPorts.add(OFPort.ofInt(1));
				arrBroadcastPorts.add(OFPort.ofInt(2));
				arrBroadcastPorts.add(OFPort.ofInt(3));
			}
			else if(sw.getId().getLong() == 3) {
				arrBroadcastPorts.add(OFPort.ofInt(1));
				arrBroadcastPorts.add(OFPort.ofInt(3));
				arrBroadcastPorts.add(OFPort.ofInt(4));
			}
			else if(sw.getId().getLong() == 4) {
				arrBroadcastPorts.add(OFPort.ofInt(1));
				arrBroadcastPorts.add(OFPort.ofInt(2));
			}
			else if(sw.getId().getLong() == 5) {
				arrBroadcastPorts.add(OFPort.ofInt(1));
				arrBroadcastPorts.add(OFPort.ofInt(2));
			}
			else
			...
			System.out.println("myLOG2 -> swID: " + sw.getId() + " Ports: " + arrBroadcastPorts);

			for (OFPort p : arrBroadcastPorts) {
				if (p.equals(inPort2)) continue;
				actions.add(sw.getOFFactory().actions().output(p, Integer.MAX_VALUE));
			}

			pob.setActions(actions);
			pob.setBufferId(OFBufferId.NO_BUFFER);
			OFMessageUtils.setInPort(pob, inPort2);
